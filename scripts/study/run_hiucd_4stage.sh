#!/bin/bash
#SBATCH --job-name=MFCS_HiUCD_4stage
#SBATCH --gres=gpu:h100:1
#SBATCH --cpus-per-task=8
#SBATCH --mem=256000M
#SBATCH --time=72:00:00
#SBATCH --output=train-hiucd-4stage-%j.out

# [ÉTUDE ablation encodeur] Hi-UCD — BASELINE 4 stages, départ ImageNet.
# Reprise automatique en cas d'annulation TIME LIMIT : resoumettre ce même script
# (l'entraînement repart depuis saved_models/study/HiUCD_4stage/last_checkpoint.pth).
...
cd ~/MambaFCS
echo "Démarrage de l'entraînement Hi-UCD (4 stages, depuis ImageNet)..."
python train.py --config configs/study/hiucd_4stage.yaml
